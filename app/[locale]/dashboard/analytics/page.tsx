...
export default async function PartnerAnalyticsPage() {
  ...
  const { data: unmetNeeds } = await supabase.from("unmet_needs_summary").select("*").limit(5)
    .limit(5)
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <AnalyticsCard
          ...
          value={stalePercent}
          description="No feedback in 90 days"
        />
        <AnalyticsCard
          title="Top Need"
          value={topNeed}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {/* Unmet Needs Table */}
        <div className="rounded-md border bg-white dark:bg-neutral-900 p-4">
            <div className="mb-4">
            ...
            <div className="relative w-full overflow-auto">
              <table className="w-full caption-bottom text-sm">
                  ...
                  <tbody className="[&_tr:last-child]:border-0">
                      {unmet.map((item) => (
                          <tr key={item.category_searched} className="border-b transition-colors hover:bg-neutral-100/50 dark:hover:bg-neutral-800/50">
                      ...
                      {unmet.length === 0 && (
                          <tr className="border-b">
                              <td colSpan={2} className="p-4 text-center text-neutral-500">No data available</td>
                          </tr>
                      )}
                  </tbody>
              </table>
              ...
              <table className="w-full caption-bottom text-sm">
                  ...
                  <tbody className="[&_tr:last-child]:border-0">
                      {((buggyServices as BuggyService[]) || []).map((item) => (
                          <tr key={item.service_id} className="border-b transition-colors hover:bg-neutral-100/50 dark:hover:bg-neutral-800/50">
                      ...
                      {(!buggyServices || buggyServices.length === 0) && (
                          <tr className="border-b">
                              <td colSpan={2} className="p-4 text-center text-neutral-500">No open issues found</td>
                          </tr>
                      )}
                  </tbody>
              </table>
