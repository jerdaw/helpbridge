...
function classifyService(service: Service): { 
  ...
  if (url.includes('.gc.ca') || url.includes('.gov.on.ca') || 
      url.includes('.canada.ca') || url.includes('ontario.ca') ||
  ...
  if (url.includes('kingstonhsc') || url.includes('hospital') || 
      name.includes('hospital') || name.includes('health centre') ||
  ...
  const establishedNonprofits = [
    'kids-help-phone', 'united-way', 'salvation-army', 'red-cross',
    'cmha', 'canadian-mental-health', 'hope-for-wellness', 'trans-lifeline',
    'assaulted-womens-helpline', 'victim-services', 'partners-in-mission'
  ]
