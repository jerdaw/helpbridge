...
async function search(query: string, location?: { lat: number, lng: number }): Promise<{ data: ScoredServicePublic[] }> {
...
async function verify() {
  ...
  assert(crisisRes.data.some((s: ScoredServicePublic) => s.id === 'crisis-988'), 'Crisis 988 should be in results')

  // Test 2: Authority Tier ("health")
  console.log('\n2️⃣ Testing Authority Tier ("health")...')
  const healthRes = await search('health')
  const govOrHealth = healthRes.data.filter((s: ScoredServicePublic) => ['government', 'healthcare'].includes(s.authority_tier || ''))
  ...
  const firstGovIdx = healthRes.data.findIndex((s: ScoredServicePublic) => ['government', 'healthcare'].includes(s.authority_tier || ''))
  ...
  if (firstCommIdx !== -1 && firstGovIdx !== -1) {
      assert(firstGovIdx < firstCommIdx, `High authority (rank ${firstGovIdx}) should outrank community (rank ${firstCommIdx})`)
  }

  // Test 3: Proximity (Kingston vs Ottawa)
  console.log('\n3️⃣ Testing Proximity ("food")...')
  // Location: Kingston City Hall
  const kingstonRes = await search('food', { lat: 44.2312, lng: -76.4860 })
  ...
  assert(firstKingston && typeof firstKingston.distance === 'number' && firstKingston.distance >= 0, 'Results should have distance calculated')
