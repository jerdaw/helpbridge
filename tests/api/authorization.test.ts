...
describe("Authorization Logic (Unit Specs)", () => {
  ...
  describe("assertServiceOwnership", () => {
    test("should throw AuthorizationError if service not found", async () => {
      ...
      await expect(assertServiceOwnership(mockSupabase as any, "user-123", "service-999")).rejects.toThrow(
        "Service not found"
      )
    })
      ...
      mockSupabase.from.mockImplementation((table) => {
        if (table === "services")
          return {
            select: vi.fn().mockReturnThis(),
            eq: vi.fn().mockReturnThis(),
            single: vi.fn().mockResolvedValue({ data: { org_id: "org-A" }, error: null }),
          }
        if (table === "organization_members") return mockMembersQuery
        return mockSupabase
      })

      await expect(assertServiceOwnership(mockSupabase as any, "user-123", "service-999")).resolves.not.toThrow()
    })
  })
  ...
  describe("assertOrganizationMembership", () => {
    test("should succeed if user has required role", async () => {
      ...
      await expect(
        assertOrganizationMembership(mockSupabase as any, "user-123", "org-A", ["owner", "admin"])
    ...
    test("should throw if user has insufficient role (viewer < editor)", async () => {
      ...
      // Expecting failure because we need 'editor', but have 'viewer'
      await expect(assertOrganizationMembership(mockSupabase as any, "user-123", "org-A", ["editor"])).rejects.toThrow(
        AuthorizationError
      )
    })
  })
