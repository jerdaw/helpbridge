...
// Mock env to enable circuit breaker
vi.mock('@/lib/env', () => ({
  env: {
    CIRCUIT_BREAKER_ENABLED: true,
    CIRCUIT_BREAKER_FAILURE_THRESHOLD: 3,
    CIRCUIT_BREAKER_TIMEOUT: 100, // Short timeout for testing
  }
}))
...
describe('Circuit Breaker Integration', () => {
  ...
  it('should recover automatically after timeout (HALF_OPEN)', async () => {
    ...
    dbSimulator.simulateFailure(3)
    for (let i = 0; i < 3; i++) {
        try { await withCircuitBreaker(mockDbOperation) } catch {}
    }
    expect(getSupabaseBreakerStats().state).toBe(CircuitState.OPEN)

    // Fast forward past timeout (100ms)
